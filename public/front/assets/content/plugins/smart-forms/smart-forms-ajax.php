...
function rednao_smart_forms_resend_email(){
    ...
    if($phpEntry->FormOptions['Emails']!=null&&count($phpEntry->FormOptions['Emails'])>$emailIndex)
    {
        ...
        $phpEntry->SendFormEmail($phpEntry->FormOptions['Emails'][$emailIndex],$phpEntry->FormEntryData,$phpEntry->ElementOptions,null,false);
    }
...
function rednao_smart_forms_entries_list()
{
    ...
    if(!isset($data->nonce)||!wp_verify_nonce($data->nonce,'smart_forms_list_entries')) {
        echo json_encode(array(
            'success' => false,
